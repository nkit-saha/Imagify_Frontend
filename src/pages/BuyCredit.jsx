...
const BuyCredit = () => {
  ...
  //       setShowLogin(true)
  //     }
  //   const {data} =  await axios.post(backendUrl+ '/api/user/pay-razor', {planId: plan}, {headers: {token}})
  //   if (data.success) {
  //     initPay(data.order)
  //   }
  //   } catch(error){
  ...
  const paymentRazorpay = async(plan) => {
  try {
    console.log("paymentRazorpay called with plan:", plan);
    ...
    const { data } = await axios.post(
      ...
      { 
        planId: plan, 
        userId: user._id   // ✅ Add this line to send userId
      }, 
      { headers: { token } }
    );
